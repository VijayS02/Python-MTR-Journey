...
<span>0.5</span> ![HK MTR System Map](mtrsystemmap "fig:") [fig:subim1]

<span>0.5</span> ![MTR System Map based on real
positions](hkroutemap "fig:") [fig:subim2]
...
<span>||c | c | c | c | c | c | c | c||</span> Line Code & Direction &
Station Code & Station ID & Chinese Name & English Name & Sequence\
[0.5ex] AEL & DT & AWE & 56 &

<span>UTF8</span><span>bsmi</span> 博覽館

& AsiaWorld-Expo & 1.00\
EAL & LMC-DT & FOT & 69 &

<span>UTF8</span><span>bsmi</span> 火炭

& Fo Tan & 7.00\
KTL & DT & MOK & 6 &

<span>UTF8</span><span>bsmi</span> 旺角
...
CSV file called *modified\_stations\_and\_lines.csv*
...
all lines on the network. It also contains an extra line “walking” which
is used for the path between Hong Kong and Central.
...
“Whampo” on the *stations\_and\_lines.csv*

Python will then use the *lines.csv* file in conjunction with the data
from *modified\_stations\_and\_lines.csv* to calculate “weights” which
...
*modified\_stations\_and\_lines\_APPENDED.csv* and *lines.csv*.
...
main methods for this class, *add\_line*, which simply adds a line code
to the station’s line codes. *add\_neighbour*, which adds a neighbour to
either airport express neighbours or regular neighbours depending on the
parameters of the method. And finally, *get\_weight* which returns the
...
\
{1: [Tung Chung station], 2: [Sunny Bay station], 3: [Tsing Yi Station],
4: [Lai King station], 5: [Nam Cheong station] ...}\
\
...
<span>0.5</span> ![Multiple Starting stations on the East Rail
Line](east_rail_multiple "fig:") [fig:subim1]

<span>0.5</span> ![Multiple stations along the Tseung Kwan O
line](tko_multiple "fig:") [fig:subim2]

[fig:image2]

The **line** class also has two methods, the first of which is the
*add\_station* method, which adds a station to the line. Next, is the
*add\_connection* method, which simply makes the 2 stations specified in
...
*add\_station* method copies the properties of the station to an
existing version if it already exists, otherwise it simply adds a new
station. The *add\_line* method simply adds the line to the lines
mapping and adds every station in the line using the *add\_station
method*.
...
1.  **Data Collection**\
    In data collection, the *data\_collection* python file is used. This
    file simply loads in the *mtr\_lines\_and\_stations.csv* file and
    then applies some processing.\
    \
    ...
    *get\_station\_coordinates* is a function which takes in the dataset
...
2.  **Information Processing**\
    The *information\_processing* file mainly contains functions that
    are used to load the data into their respective objects and is used
    to generate a System map from the csv data provided. It does this by
...
3.  **Mapping** This python file is used to inform python where the
    ...
    writes out this data to a file titled *coord\_mappings.csv*. See
...
4.  **Main Program** The main program that runs the interactive UI
    ...
    the *mtr\_lines\_fares.csv* and a shortest path is made. The program
    ...
    NOTE: if *run\_main* is run with the last parameter
    (*show\_station\_boxes*) set to true, then the click boxes of each
...
*mtr\_lines\_and\_stations.csv* and *mtr\_lines\_fares.csv* are csv
files downloaded from the MTR website.
*modified\_mtr\_lines\_and\_stations.csv*,
<span>coord\_mappings.csv</span> and
*modified\_mtr\_lines\_and\_stations\_APPENDED.csv* are automatically
generated by the programs if executed in the order described by
computational overview. (i.e. data\_collection.py mapping.py main.py).
The rest have been manually made by me.
...
console. In addition, the *run\_main* function’s call in the
...
constants and *run\_main* for more details).
...
online Available at:\
https://commons.wikimedia.org/wiki/File:Hong\_Kong\_Railway\_Route\_Map\_blank.svg
...
http://www.mtr.com.hk/en/customer/services/system\_map.html [Accessed 13
March 2021].

Data.gov.hk. 2021. | DATA.GOV.HK. online Available at:\
https://data.gov.hk/en-datasets/search/MTR [Accessed 13 March 2021].

https://en.wikipedia.org/wiki/Dijkstra https://www.pygame.org/docs/
